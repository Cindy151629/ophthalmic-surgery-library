...
每篇笔记分别记录研究问题、设计与对象、方法、具体结果、证据解读、局限和来源定位。已有实质笔记、证据仍待补充、尚未取得实质内容分别统计；摘要笔记可以详尽，但始终明确其摘要范围。下载正文不等于已经阅读全文，题录或访问失败说明不计为详细笔记。实时逐篇核查由 `scripts/audit_notes.py` 执行，生成 `reports/note-coverage.json` 与网页制品 `note-audit.json`，并将数量写入 `manifest.json`。
